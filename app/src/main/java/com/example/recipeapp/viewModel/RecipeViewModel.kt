...
class RecipeViewModel(var context: Context) : ViewModel() {
    ...
    var allRecipes: ArrayList<Recipe> = createSampleRecipes()
    ...
    fun filterRecipesByType(recipeTypeId: Int) {
     ...
     private fun createSampleRecipes(): ArrayList<Recipe> {
        recipes.add(Recipe(1, "Scrambled Eggs",
            ...
            getResourceUri(context ,R.mipmap.egg)))
        recipes.add(Recipe(2, "Big Breakfast", "1099 calories per serve", 1, "Step 1\n" +
                ...
                "- 2 red onions, cut into wedges", getResourceUri(context ,R.mipmap.big)))
    ...
    fun getLatestRecipeList(): LiveData<List<Recipe>> {
        filteredRecipesLiveData = _filteredRecipesLiveData
